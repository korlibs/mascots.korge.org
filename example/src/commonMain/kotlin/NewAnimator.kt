...
open class NewAnimator(
    ...
    ) = NewAnimator(root, time, speed, easing, completeOnCancel, true, looped).also(callback).also { addNode(it) }
    ...
    inline fun sequence(
        ...
        looped: Boolean = false,
        callback: @NewAnimatorDslMarker NewAnimator.() -> Unit
    ) = NewAnimator(root, time, speed, easing, completeOnCancel, false, looped).also(callback).also { addNode(it) }
    ...
    ) = NewAnimator(root, time, speed, easing, completeOnCancel, true, looped).also(init).also { addNode(it) }
    ...
    fun sequenceLazy(
        ...
        looped: Boolean = false,
        init: @NewAnimatorDslMarker NewAnimator.() -> Unit
    ) = NewAnimator(root, time, speed, easing, completeOnCancel, false, looped).also(init).also { addNode(it) }
...
fun <T : View> T.addUpdater2(first: Boolean = true, updatable: T.(dt: TimeSpan) -> Unit): UpdateComponent {
